...
import { publicListingVendors } from "../../data/publicListingVendors";

function SearchVendor({ routeTitle = "Search Results" }) {
    const app = homepageAppState;
    ...
    const tryFilterVendors = async (params, page = 1) => {
        const normalizedCategoryId = params.categoryId ? Number(params.categoryId) : null;
        const filteredVendors = publicListingVendors.filter((vendor) => {
            const matchesCategory = normalizedCategoryId ? vendor.categoryId === normalizedCategoryId : true;
            const matchesName = params.name ? vendor.fullName.toLowerCase().includes(String(params.name).toLowerCase()) : true;
            return matchesCategory && matchesName;
        });
        const start = (page - 1) * limit;
        const paginatedVendors = filteredVendors.slice(start, start + limit);
        setVendors(paginatedVendors);
        setTotalPages(Math.max(1, Math.ceil(filteredVendors.length / limit)));
        setCurrentPage(page);
        ...
        <>
            <div className="search-vendor-bg">
                {filters.categoryId ? <div className="mb-3 text-center">
                    ...
                    <h2 className="heading-text">{routeTitle}</h2>
                </div>}
                <GlobalSearch
                    app={app}
                    filter={filters}
                                ...
                                <div className="wedding-listing">
                                    <div className="img relative">
                                        <NavLink to={`/vendor-detail/${vendor.id}`}>
                                            {vendor.photo ? <img src={vendor.photo?.startsWith('/assets/') ? vendor.photo : `${app.serverPath + vendor.photo}`} alt={vendor.fullName} className="card-image" /> : <img src="/assets/images/placeholder.png" alt={vendor.fullName} className="card-image" />}
                                        </NavLink>
